...
void dsuInit (int n, vector<int>& parent, vector<int>& size)
{
    for (int i = 0; i < n; ++i)
    {
...
void solve()
{
    ...
    while (q--)
    {
        ...
        switch (queryType)
        {
            case 0: union_sets (a, b, parent, size);
                    break;
                    
            case 1: cout << (find_set (a, parent, size) == find_set (b, parent, size)) << endl;
                    break;
        }
    }
}
